...
const binp = path.join(cargoDir, "bin", "arxiv-cli");

if (fs.existsSync(binp)) {
  console.log("Uninstalling arxiv-cli...");
  exec(`cargo uninstall arxiv-cli`, (error, stdout, stderr) => {
    console.log(stdout);
    if (error || stderr) {
  ...
  console.log("arxiv-cli not found skipping!");
}
