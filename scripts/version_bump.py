...
def new_version_to_pre_install_js(old_version: str, new_version: str) -> None:
    with open("pre-install.js", "r") as f:
        content = f.read()
    old_v_log = f"Installing and compiling arxiv-cli {old_version}"
    old_v_install = f"cargo install arxiv-cli --vers {old_version}"
    new_v_log = f"Installing and compiling arxiv-cli {new_version}"
    new_v_install = f"cargo install arxiv-cli --vers {new_version}"
...
def main() -> None:
    ...
    assert compare_semver(semv_rust, semv_js), f"Version {v_js} (package.json) and {v_rust} (Cargo.toml) do not match!"
    bump_type = ""
    while True:
        bump_type = input("What type of bump do you want to perform? [major/minor/patch] ")
